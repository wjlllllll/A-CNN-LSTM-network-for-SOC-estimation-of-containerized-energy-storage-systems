...
history = model1.fit(X_train, y_train, epochs=1, batch_size=256, validation_data=(X_val, y_val), callbacks=[early_stopping, model_checkpoint])

# 预测数据
test_data = scaled_data[415800:, :]
X_test = []
y_test = []
for i in range(60, len(test_data)):
    X_test.append(test_data[i-60:i, :])
    y_test.append(data['soc'][415800+i-1])
...
model2 = Sequential()
model2.add(GRU(units=32, return_sequences=True, input_shape=(X_train.shape[1], X_train.shape[2])))
model2.add(Flatten())
...
# 创建模型
model5 = Sequential()
model5.add(LSTM(units=32, input_shape=(X_train.shape[1], X_train.shape[2])))
...
df_mape1 = df_mape1.append({'Model': '1', 'MAPE': mape1}, ignore_index=True)
...
df_mape2 = df_mape1.append({'Model': '2', 'MAPE': mape1}, ignore_index=True)
...
df_mape3 = df_mape1.append({'Model': '3', 'MAPE': mape1}, ignore_index=True)
